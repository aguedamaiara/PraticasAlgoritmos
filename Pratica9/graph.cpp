...
void Graph::edge(int src, int dst, int w) {
	...
	adj[src].push_back(newEdge);  // Adicionar a nova aresta à lista de adjacência de src
}
...
int Graph::weight(int src, int dst) {
	...
	// Caso não haja uma aresta entre src e dst, retorne -1 ou outro valor adequado
	return -1;
}
