...
public:
  ...
  void insert(T_key key, T_value value) {
    int hash_inserido = hash(key);
    Node *&inserido = data[hash_inserido];
    Node *atual = inserido;
   
    __insert(inserido, key, value);
    size++;
  }
  ...
  T_value remove(T_key key) {
    ...
    int hash_removido = hash(key);
    Node *&removido = data[hash_removido];
    T_value retirado = __remove(removido, key);
    if (retirado != notFound) {
      size--;
    }
    return retirado;
  }
  ...
  T_value search(T_key key) {
    ...
    int hash_search = hash(key);
    Node *&procura = data[hash_search];
    return __search(procura, key);
  }
